...
def extractFemaleNames(filename,letter):
  ...
  line = re.findall('<td>(\d+)</td><td>(\w+)</td>\<td>(\w+)</td>',read_line,re.IGNORECASE)
  
  for l in line:
    if l[2][0] == letter:
      FemalesNames.append(str(l[2]) + ' ' + str(l[0]))
...
def extractMaleNames(filename,letter):
  ...
  line = re.findall(r'<td>(\d+)</td><td>(\w+)</td>\<td>(\w+)</td>',read_line,re.IGNORECASE)
  
  for l in line :
    if l[1][0] == letter :
      MaleNames.append(str(l[1]) + ' ' + str(l[0]))
  
  MaleNames.sort(reverse = True)
...
def extractDocAndPopularityDate(filename):
  ...
  DocumentDate   = re.findall(r'<h1>+[\w+\s]+\w+<\Sh1>([\w+\s+,]+)+[\w+\S]',line)
  PopularityDate = re.findall(r'<h3\salign="center">([\w\s]+)<\Sh3>',line)
  
  return str(str(DocumentDate[0]) +' and '+ str(PopularityDate[0]))
