...
def fix_start(s,filler):
  # +++your code here+++
  first_char = s[0]                        # Taking out the first character 
  s = s[1:]                                # Taking out the rest of characters 
  replaced = s.replace(first_char,filler)  # Replacing Like first character with filler
  s = first_char + replaced                # Concatination
...
def mix_up(a, b):
  ...
  if len(a) and len(b) > 2:
    s1 = a[0:2] + b[2:]         # s1 = do nner
    s2 = b[0:2] + a[2:]         # s2 = di g 
    joined = '-'.join([s2,s1])
  return joined
